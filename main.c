...
static void module_init() {
    ...
    for (x = 0; x < LUA_STATES; x++) {
        if (!Lua_states.states[x].state) {
            Lua_states.states[x].state = luaL_newstate();
            Lua_states.states[x].sessions = 0;
            L = (lua_State *) Lua_states.states[x].state;
            lua_pushinteger(L, x);
            ...
            for (y = 0; y < 100; y++) {
                memset(Lua_states.states[x].files[y].filename, 0, 512);
                Lua_states.states[x].files[y].modified = 0;
                Lua_states.states[x].files[y].refindex = 0;
            }
        }
    }
}
...
int lua_parse_file(lua_thread* thread, char* input) {
    ...
    rc = luaL_loadstring(thread->state, output ? output : "echo('(nil)');");
    return rc;
}
...
static int plua_handler(request_rec* r)
{
    int exists = 1;
    struct stat statbuf;
    if (!r->handler || strcmp(r->handler, "plua"))
        return DECLINED;
    ...
    else if S_ISDIR(statbuf.st_mode) exists = 0;
    ...
    if (!exists) {
        ...
        if (rc < 1) {
            ap_set_content_type(r, "text/html;charset=ascii");
            ap_rprintf(r, "<b>Compiler error in %s:</b> %s!", r->filename, lua_tostring(L, -1));
        }
        else { 
            /*$2
         ------------------------------------------------------------------------------------------------
         ...
         */
            ...
            if (lua_pcall(L, 0, LUA_MULTRET, 0)) {
                ap_set_content_type(r, "text/html;charset=ascii");
                ap_rprintf(r, "<b>Lua error:</b> %s!", lua_tostring(L, -1));
            }
            else {
