/*$T indentinput.c GC 1.140 03/05/12 16:41:59 */
...
static int plua_handler(request_rec *r)
{
	...
	if(!exists)
		...
		if(rc < 1) {
			...
			/*
			 * If script timeout or memory limit is in effect, attach the debug hook for
			 * monitoring.
			 */
	...
	if(err && !strstr(err, "MOD_PLUA_EXIT")) {
		...
		for(x = 1; x < 2048; x++) {
			...
			if((where = strstr(errX, found)) != 0) {
				...
				if(lineX) {
					sprintf(lineX,
							"On line <code style='font-weight: bold; color:#774411;'>%u:</code> %s",
						x, where + strlen(found));
					errX = lineX;
				}
				break;
			}
		...
		if(LUA_LOGLEVEL >= 3)
			ap_log_rerror(filename, 0, APLOG_ERR, APR_EGENERAL, thread->r,
						  "in %s: %s", filename, err);
#else
		if(LUA_LOGLEVEL >= 3)
			ap_log_rerror(filename, 0, 0, APLOG_ERR, APR_EGENERAL, thread->r,
						  "in %s: %s", filename, err);
#endif
		ap_set_content_type(thread->r, "text/html; charset=ascii");
		filename = filename ? filename : "";
		ap_rprintf(thread->r, pLua_error_template, type, filename ? filename : "??",
				   errX ? errX : err);
	}
}
	...
	if((then % 200) == 0) {
		...
		if(LUA_TIMEOUT > 0) {
			...
			if(thread && (now - thread->runTime) > LUA_TIMEOUT)
				luaL_error(L,
						   "The script took too long to execute (timed out)!\n",
		...
		if(LUA_MEM_LIMIT > 0) {
			...
			if(kb > LUA_MEM_LIMIT) {
				...
				if(kb > LUA_MEM_LIMIT)
					luaL_error(L, "Memory limit reached!\n",
							   "the script...somewhere!");
			}
		}
	}
...
int lua_parse_file(lua_thread *thread, char *input)
{
	...
	int			rc = 0;
	char		*output = 0;
	char		*matchStart, *pmatchStart, *matchEnd;
	size_t		at = 0;
	...
	while(at < inputSize) {
		...
		if(matchStart) {
			...
			if(matchStart - input > 0) {
				...
				/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
				char	*test = (char *) apr_pcalloc(thread->r->pool,
													 matchStart - input + at + 20);
				/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

				X = matchStart[0];
				matchStart[0] = 0;
#ifdef _WIN32
				_snprintf_c(test, matchStart - input + 18,
							"echo([===[%s]===]);", input + at);
#else
				snprintf(test, matchStart - input + 18, "echo([===[%s]===]);",
						 input + at);
#endif
				matchStart[0] = X;
			...
			if(matchEnd) {
				...
				if(matchStart[strlen(sTag)] == '=') {
					test = (char *) apr_pcalloc(thread->r->pool,
												matchEnd - matchStart + strlen(sTag) + 10);
					etest = (char *) apr_pcalloc(thread->r->pool,
												 matchEnd - matchStart + 20);
					strncpy(test, matchStart + 1 + strlen(sTag),
							matchEnd - matchStart - strlen(sTag) - 1);
					...
					test = (char *) apr_pcalloc(thread->r->pool,
												matchEnd - matchStart + strlen(sTag) + 10);
					strncpy(test, matchStart + strlen(sTag),
							matchEnd - matchStart - strlen(sTag));
...
#ifdef _WIN32
				_snprintf(test, strlen(input) - at + 18, "echo([===[%s]===]);",
						  input + at);
#else
				snprintf(test, strlen(input) - at + 18, "echo([===[%s]===]);",
						 input + at);
#endif
				at = inputSize;
				lua_add_code(&output, test);
	...
	)
{
	...
	for(x = 0; x < LUA_FILES; x++) {
		...
		if(!strcmp(thread->files[x].filename, filename)) {
			...
			if
			(
				statbuf->mtime != thread->files[x].modified
			||	statbuf->size != thread->files[x].size
			) {
				memset(thread->files[x].filename, 0, 256);
				luaL_unref(thread->state, LUA_REGISTRYINDEX,
						   thread->files[x].refindex);
				break;
		...
		if(input) {
			...
			if(!rawCompile) {
				...
				if(iBuffer) {
					...
					rc = luaL_loadstring(thread->state,
										 "echo('No input specified or file was written to at time of execution')");
				}
			}
...
char *getPWD(lua_thread *thread)
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	size_t	x, y, z;
	char	*pwd = (char *) apr_pcalloc(thread->r->pool,
										strlen(thread->r->filename) + 2);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
static int lua_dbescape(lua_State *L)
{
	...
	if(thread) {
		...
		if(db && db->alive) {
			...
			escaped = apr_dbd_escape(db->driver, thread->r->pool, statement,
									 db->handle);
			if(escaped) {
...
static int lua_dbquery(lua_State *L)
{
	...
	if(thread) {
		...
		if(db && db->alive) {
			...
			apr_dbd_init(thread->r->pool);
			rc = apr_dbd_select(db->driver, thread->r->pool, db->handle,
								&results, statement, 0);
			if(rc == APR_SUCCESS) {
				...
				if(cols > 0) {
					...
					(
						rows = 1;
						apr_dbd_get_row(db->driver, thread->r->pool, results,
										&row, rows) != -1;
						rows++
					) {
	...
	if(thread) {
		...
		if(!strcmp(type, "mod_dbd")) {
			...
			if(rc == APR_SUCCESS) {
				...
				lua_settop(L, 0);
				if(strlen(arguments)) {
					rc = apr_dbd_open_ex(db->driver, db->pool, arguments,
										 &db->handle, &error);
					if(rc == APR_SUCCESS) {
						db->alive = 1;
						lua_newtable(L);
				...
				lua_pushfstring(thread->state,
								"The database driver for '%s' could not be found!",
								type);
	...
	if(thread) {
		...
		for(y = 1; y < z; y++) {
			...
			if(t == LUA_TNUMBER || t == LUA_TSTRING) {
				...
				if(t == LUA_TBOOLEAN)
					el = (lua_toboolean(L, y)) ? "true" : "false";
				if(t == LUA_TFUNCTION) {
					sprintf(string, "function [%p]", lua_topointer(L, y));
					el = string;
				}

				if(t == LUA_TTABLE) {
					sprintf(string, "table [%p]", lua_topointer(L, y));
					el = string;
				}

				if(t == LUA_TUSERDATA || t == LUA_TLIGHTUSERDATA) {
					sprintf(string, "userdata [%p]", lua_topointer(L, y));
					el = string;
				}
...
static int lua_getEnv(lua_State *L)
{
	...
	if(thread) {
		...
		sprintf(luaVersion, "%u.%u", (LUA_VERSION_NUM / 100),
				(LUA_VERSION_NUM) % (LUA_VERSION_NUM / 100));
		...
		sprintf(http_version, "%u.%u.%u", version.major, version.minor,
				version.patch);
...
static int lua_getRequestBody(lua_State *L)
{
	...
	if(thread) {
		...
		if
		(
			thread->r->method_number != M_POST
		&&	thread->r->method_number != M_PUT
		) return(0);
		if(!filename) {
			...
			rc = apr_file_open(&file, filename, APR_CREATE | APR_FOPEN_WRITE,
							   APR_FPROT_OS_DEFAULT, thread->r->pool);
			lua_settop(L, 0);
			if(rc == APR_SUCCESS) {
		...
		)
{
	...
	for(z = 0; z < MAX_VARS - 1; z++) {
		if(form[z].key) {
			if(form[z].size == 1) {
				lua_pushstring(thread->state, form[z].key);
				lua_pushlstring(thread->state, form[z].values[0],
								form[z].sizes[0]);
				lua_rawset(thread->state, -3);
			}
			else {
				...
				for(i = 0; i < form[z].size; i++) {
					lua_pushinteger(thread->state, i + 1);
					lua_pushlstring(thread->state, form[z].values[i],
									form[z].sizes[i]);
					lua_rawset(thread->state, -3);
				}
...
static int lua_includeFile(lua_State *L)
{
	...
	if(thread) {
		...
		rc =
			(
				(fileinfo.filetype != APR_NOFILE)
			&&	!(fileinfo.filetype & APR_DIR)
			);
		if(!rc)
			lua_pushboolean(L, 0);
		else {
...
void pLua_init_states(lua_domain *domain)
{
	...
	domain->states = (lua_thread *) apr_pcalloc(domain->pool,
												(LUA_STATES + 1) * sizeof(lua_thread));
	y = (LUA_STATES + 1) * sizeof(lua_thread);
	if(LUA_LOGLEVEL >= 2)
	{
#if (AP_SERVER_MINORVERSION_NUMBER <= 2)
		ap_log_perror("mod_plua.c", 2456, APLOG_NOTICE, -1, domain->pool,
					  "Allocated new domain pool for '%s' of size %u (%u states)",
					  domain->domain, y, LUA_STATES);
#else
		ap_log_perror("mod_plua.c", 2456, 1, APLOG_NOTICE, -1, domain->pool,
					  "Allocated new domain pool for '%s' of size %u (%u states)",
					  domain->domain, y, LUA_STATES);
#endif
	}
	...
	for(y = 0; y < LUA_STATES; y++) {
		if(!domain->states[y].state) {
			domain->states[y].files = apr_pcalloc(domain->pool,
												  (LUA_FILES + 1) * sizeof(pLua_files));
...
lua_thread *lua_acquire_state(request_rec *r, const char *hostname)
{
	...
	if(LUA_MULTIDOMAIN > 0) {
		...
		if(!domain) {
			...
			{
#if (AP_SERVER_MINORVERSION_NUMBER <= 2)
				ap_log_perror("mod_plua.c", 2495, APLOG_NOTICE, APR_ENOPOOL,
							  LUA_BIGPOOL,
							  "mod_pLua: Domain pool too small, reallocating space for new domain pool '%s' of size %u bytes <Not an Error>",
						  hostname, (uint32_t) sizeof(lua_domain) *
								  (pLua_domainsAllocated + 3));
#else
				ap_log_perror("mod_plua.c", 2495, 1, APLOG_NOTICE, APR_ENOPOOL,
							  LUA_BIGPOOL,
							  "mod_pLua: Domain pool too small, reallocating space for new domain pool '%s' of size %u bytes <Not an Error>",
						  hostname, (uint32_t) sizeof(lua_domain) *
								  (pLua_domainsAllocated + 3));
			...
			pLua_domains = (lua_domain *) realloc(pLua_domains,
												  sizeof(lua_domain) * (pLua_domainsAllocated + 2));
			if(pLua_domains == 0) {
				if(LUA_LOGLEVEL >= 1)
				{
#if (AP_SERVER_MINORVERSION_NUMBER <= 2)
					ap_log_perror("mod_plua.c", 2500, APLOG_CRIT, APR_ENOPOOL,
								  LUA_BIGPOOL,
								  "mod_pLua: Realloc failure! This is bad :(");
#else
					ap_log_perror("mod_plua.c", 2500, 1, APLOG_CRIT,
								  APR_ENOPOOL, LUA_BIGPOOL,
								  "mod_pLua: Realloc failure! This is bad :(");
#endif
				}
...
char *pLua_sha256(const char *digest, lua_thread *thread)
{
	...
	sprintf(ret, "%08x%08x%08x%08x%08x%08x%08x%08x", shaX[0], shaX[1], shaX[2],
			shaX[3], shaX[4], shaX[5], shaX[6], shaX[7]);
	return(ret);
}
...
static int lua_b64dec(lua_State *L)
{
	...
	if(thread) {
		if(ilen) {
			...
			olen = pLua_unbase64((unsigned char *) output,
								 (const unsigned char *) string, ilen);
			lua_settop(L, 0);
